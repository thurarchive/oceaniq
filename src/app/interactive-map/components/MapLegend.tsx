  ...
  return (
    <div ref={legendRef} className="absolute bottom-4 right-4 glass-card-elevated border border-border rounded-xl p-3 z-20 min-w-48 shadow-2xl animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-semibold text-foreground uppercase tracking-wider">{t.map.legend}</p>
        <button
      ...
      <div className="space-y-2 mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground">
            {language === 'id' ? 'Observasi Terverifikasi' : 'Verified Observation'}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground">
            {language === 'id' ? 'Laporan Sains Warga' : 'Citizen Report'}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground">
            {language === 'id' ? 'Estimasi Model AI' : 'ML Estimate'}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground">
            {language === 'id' ? 'Stasiun Cuaca Maritim' : 'Weather Station'}
          </span>
        </div>
      </div>
      ...
      <div className="border-t border-border pt-2.5 mb-2.5">
        <p className="text-xs font-semibold text-muted-foreground mb-1.5 uppercase tracking-wide text-[10px]">
          {language === 'id' ? 'Kuantitas Sampah' : 'Debris Quantity'}
        </p>
        <div className="space-y-1">
          {[
            { label: language === 'id' ? 'Kritis (> 1000 item)' : 'Critical (> 1000 items)', color: 'bg-danger', size: 'w-2.5 h-2.5' },
            { label: language === 'id' ? 'Tinggi (500-1000 item)' : 'High (500-1000 items)', color: 'bg-warning', size: 'w-2.5 h-2.5' },
            { label: language === 'id' ? 'Sedang (200-500 item)' : 'Medium (200-500 items)', color: 'bg-primary', size: 'w-2.5 h-2.5' },
            { label: language === 'id' ? 'Rendah (< 200 item)' : 'Low (< 200 items)', color: 'bg-positive', size: 'w-2.5 h-2.5' },
          ].map((item) => (
      ...
      <div className="border-t border-border pt-2.5">
        <p className="text-xs font-semibold text-muted-foreground mb-1.5 uppercase tracking-wide text-[10px]">
          {language === 'id' ? 'Zona Hujan (BMKG)' : 'Rainfall Buffer'}
        </p>
